...
import { Search, Download, X, Eye, Play, Calendar as CalendarIcon } from 'lucide-react';
...
const Orders = () => {
  ...
  const filteredOrders = orders.filter(order => {
    const matchesStatus = 
      (selectedStatus === 'ongoing' && (order.status === 'pending' || order.status === 'confirmed')) ||
    ...
    const matchesOrderType = filterOrderType === 'all' || order.order_type === filterOrderType;
    const matchesRestaurant = filterRestaurant === 'all' || order.restaurant_id === filterRestaurant;
    ...
    return matchesStatus && matchesSearch && matchesOrderType && matchesRestaurant && matchesDateRange;
  });
  ...
  useEffect(() => {
    setCurrentPage(1);
  }, [selectedStatus, searchQuery, filterOrderType, filterRestaurant, startDate, endDate]);
  ...
  return (
    <div className="p-6 space-y-4">
      {/* Header */}
      <div>
        <h1 className="text-2xl font-bold text-gray-900">All Orders Placed</h1>
        <p className="text-sm text-gray-600 mt-1">Dashboard &gt; All Orders</p>
      </div>
      ...
      <div className="bg-white rounded-lg border border-gray-200 p-4 space-y-3">
        <div className="flex gap-4 items-center flex-wrap">
          ...
          <button
            ...
            className="px-4 py-2 bg-green-600 hover:bg-green-700 text-white text-sm font-medium rounded-md flex items-center gap-2"
          >
        ...
        <div className="flex gap-3 items-center flex-wrap">
          ...
          {restaurants.length > 0 && (
            <div className="flex items-center gap-2">
              <label className="text-xs text-gray-600">Restaurant:</label>
              <select
                value={filterRestaurant}
                onChange={(e) => setFilterRestaurant(e.target.value)}
                className="px-3 py-1.5 border border-gray-300 rounded-md text-sm focus:outline-none focus:ring-2 focus:ring-green-500 min-w-[150px]"
              >
          ...
          {(filterOrderType !== 'all' || filterRestaurant !== 'all' || startDate || endDate) && (
            <button
              onClick={() => {
              ...
              className="px-3 py-1.5 text-xs text-gray-600 hover:text-gray-900 border border-gray-300 rounded-md hover:bg-gray-50"
            >
                    ...
                    <td className="py-3 px-4">
                      <div className="flex gap-2">
                        {order.status === 'pending' && (
                          <button
                            onClick={() => navigate(`/restaurant/pos?resume=${order.id}`)}
                            className="px-2 py-1 bg-green-600 hover:bg-green-700 text-white text-xs rounded-md flex items-center gap-1"
                          >
