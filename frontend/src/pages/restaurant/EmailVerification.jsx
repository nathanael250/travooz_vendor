...
export default function EmailVerification() {
  ...
  const { userId: stateUserId, email: stateEmail, userName: stateUserName, restaurantId: stateRestaurantId } = location.state || {};
  ...
  const getRestaurantId = () => {
    if (stateRestaurantId) return stateRestaurantId;
  ...
  // Store restaurantId in localStorage if available from state
  useEffect(() => {
    if (stateRestaurantId) {
      localStorage.setItem('restaurant_id', stateRestaurantId);
      console.log('EmailVerification - Stored restaurantId from state:', stateRestaurantId);
    } else {
      // Check localStorage as fallback
      const storedRestaurantId = localStorage.getItem('restaurant_id');
      if (storedRestaurantId) {
        console.log('EmailVerification - Found restaurantId in localStorage:', storedRestaurantId);
      } else {
        console.warn('EmailVerification - No restaurantId found in state or localStorage. State:', location.state);
      }
    }
  }, [stateRestaurantId, location.state]);
  ...
  useEffect(() => {
    const sendInitialCode = async () => {
      ...
      try {
        await restaurantSetupService.sendEmailVerificationCode(userId, email, userName);
      } catch (error) {
  ...
  const handleVerify = async (e) => {
    ...
    setIsVerifying(true);
    try {
      const response = await restaurantSetupService.verifyEmailCode(userId, email, verificationCode, restaurantId);
      
      if (response.verified || response.success) {
        setIsVerified(true);
        
        // Ensure restaurantId is available (check localStorage if missing from state)
        let finalRestaurantId = restaurantId;
        if (!finalRestaurantId) {
          finalRestaurantId = localStorage.getItem('restaurant_id');
          console.log('EmailVerification - Retrieved restaurantId from localStorage:', finalRestaurantId);
        }
        ...
        // Redirect to business details step after 2 seconds
        setTimeout(() => {
