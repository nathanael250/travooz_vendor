...
export default function ListYourTourStep3() {
  ...
  useEffect(() => {
    const checkUserService = async () => {
      if (user && isVendor) {
        ...
        try {
          // Try to get profile from tours service
          // If user exists in tours_users, they belong to this service
          ...
          // If we get a successful response, user exists in tours_users
          ...
          // If 404 or "User not found", user doesn't exist in tours_users
  ...
  const handleSubmit = async (e) => {
    ...
    if (isVendor) {
      // User is already logged in, create tour business listing only
      ...
      try {
        ...
        // Navigate to business owner information step (Step 4)
        navigate('/tours/setup/business-owner-info', {
          state: {
            ...
            email: user.email,
            userName: user.name,
            tourBusinessId: tourBusinessId
          }
        });
      } catch (error) {
        console.error('Error creating tour business listing:', error);
        setSubmitError(error.response?.data?.message || error.message || 'Failed to create tour business listing. Please try again.');
        toast.error(error.response?.data?.message || error.message || 'Failed to create tour business listing');
      } finally {
    ...
    try {
      ...
      toast.success('Account and tour business listing created successfully!');
                  ...
                  <div className="p-4 bg-yellow-50 rounded-lg border border-yellow-300 flex items-start gap-3">
                    ...
                    <div className="flex-1">
                      ...
                      <p className="text-xs text-yellow-800 mb-3">
                        ...
                        To create a new tour package account with the same email, please logout first.
                      </p>
                      <button
                        type="button"
                        onClick={() => {
                          localStorage.clear();
                          window.location.href = '/tours/list-your-tour';
                        }}
                  ...
                  <div className="p-4 bg-green-50 rounded-lg border border-green-200 flex items-start gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-green-700 mt-2">
                        We'll use your existing account to complete the tour business registration.
                      </p>
                    </div>
                  </div>
                )}
              ...
              <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <div className="bg-gray-50 rounded-lg p-4 border" style={{ borderColor: '#e5e7eb' }}>
                  <p className="text-xs text-gray-600">
                    By continuing, you agree to allow Travooz to contact you regarding your tour business registration, including via text message.
                  </p>
                </div>
