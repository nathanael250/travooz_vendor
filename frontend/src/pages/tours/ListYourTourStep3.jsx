...
export default function ListYourTourStep3() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!step2Data || Object.keys(step2Data).length === 0) {
      toast.error('Please provide your tour business information first.');
      return;
    }
    ...
    if (!isVendor) {
      ...
      if (!formData.firstName.trim()) {
        newErrors.firstName = 'Cannot be left blank';
      }

      if (!formData.lastName.trim()) {
        newErrors.lastName = 'Cannot be left blank';
      }
    ...
    try {
      // Prepare the payload for tour business creation
      const payload = {
        // Location data from Step 1
        location: selectedLocation,
        locationData: selectedLocationData,
        // Tour business data from Step 2
        tourBusinessName: step2Data.tourBusinessName || '',
        selectedTourTypeNames: step2Data.selectedTourTypeNames || [],
        selectedTourTypes: step2Data.selectedTourTypes || [],
        description: step2Data.description || '',
        phone: formData.phone || step2Data.phone || '',
        countryCode: formData.countryCode || step2Data.countryCode || '+250',
      ...
      const apiResponse = await tourPackageSetupService.createTourListing(payload);
      const responseData = apiResponse?.data || apiResponse || {};

      const tourBusinessId = responseData.tourBusinessId || responseData.tour_business_id || responseData.data?.tourBusinessId;
      const userId = responseData.userId || responseData.user_id || responseData.data?.userId;
      const token = responseData.token || responseData.data?.token;

      if (!tourBusinessId || !userId) {
      ...
      const createdUser = {
        ...
        name: derivedName,
        email: formData.email,
        phone: formData.phone || step2Data.phone || null,
        tour_business_id: tourBusinessId,
        tour_business_status: responseData.status || 'draft'
      };
      localStorage.setItem('user', JSON.stringify(createdUser));
      localStorage.setItem('tour_business_id', tourBusinessId);
      localStorage.setItem('service_type', 'tours');
      setUser(createdUser);

      toast.success('Tour business registration saved successfully!');
      ...
      if (errorData.errors) {
        ...
        setSubmitError(errorData.message || 'Failed to create tour business. Please try again.');
      }

      toast.error(errorData.message || 'Failed to create tour business');
    } finally {
      setIsSubmitting(false);
    }
  };
        ...
        <div className="max-w-2xl w-full mx-auto">
          ...
          <div className="bg-white rounded-lg shadow-xl p-8 border" style={{ borderColor: '#dcfce7' }}>
            ...
            <p className="text-center text-gray-600 mb-8">
              Sign in to set up your new tour business if you already have an account.{' '}
              <button
                type="button"
