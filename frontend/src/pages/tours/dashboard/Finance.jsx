...
import { DollarSign, FileText, CheckCircle, CreditCard, Download, Info, X, Edit, Plus } from 'lucide-react';
...
const Finance = () => {
  ...
  const fetchBookingsForPayout = async () => {
    try {
      const response = await apiClient.get('/tours/bookings?status=confirmed,completed');
  ...
  const fetchInvoices = async () => {
    try {
      const response = await apiClient.get(`/tours/invoices?year=${selectedYear}`);
  ...
  const fetchPaymentConfirmations = async () => {
    try {
      const response = await apiClient.get(`/tours/payment-confirmations?year=${selectedYear}`);
      ...
      const bookingsResponse = await apiClient.get('/tours/bookings?status=confirmed,completed');
      const bookings = bookingsResponse.data?.data || bookingsResponse.data || [];
  ...
  const fetchPaymentTaxDetails = async () => {
    try {
      ...
      if (businessId) {
        const response = await apiClient.get(`/tours/businesses/${businessId}/payment-tax`);
                ...
                <table className="min-w-full divide-y divide-gray-200">
                  <thead className="bg-gray-50">
                    <tr>
                      ...
                      <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Lead traveler</th>
                      <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Product code</th>
                      <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Option code</th>
                      <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Activity date</th>
                    ...
                    {bookingsForPayout.length === 0 ? (
                      ...
                      bookingsForPayout.map((booking) => (
                        <tr key={booking.booking_id || booking.id} className="hover:bg-gray-50">
                          ...
                          <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                            {booking.option_reference_code || 'N/A'}
                          </td>
                          <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
