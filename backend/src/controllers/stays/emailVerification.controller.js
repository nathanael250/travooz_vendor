const emailVerificationService = require('../../services/stays/emailVerification.service');
...
const sendVerificationCode = async (req, res) => {
    try {
        ...
        // Check rate limit
        const isRateLimited = await emailVerificationService.checkRateLimit(email);
        if (isRateLimited) {
            return sendError(
                res,
...
const verifyCode = async (req, res) => {
    try {
        ...
        // Verify code
        const isValid = await emailVerificationService.verifyCode(userId, email, code);
