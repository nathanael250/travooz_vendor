-- Travooz Tours Database Schema
-- Database: travoozapp_db (shared database)
-- All tables use 'tours_' prefix to avoid conflicts
--
-- IMPORTANT: These tables are created in the existing travoozapp_db database
-- Make sure you are connected to travoozapp_db before running this script
-- USE travoozapp_db;
--
-- Or run: mysql -u username -p travoozapp_db < tours_db.sql
...
-- Stores main tour business listing information
-- Created in Step 1 (Location + Basic Info + Account Creation)
-- ============================================
...
-- Stores email verification codes for tour businesses
-- Used in Step 1 (Email Verification)
-- ============================================
...
-- Stores business owner personal information
-- Created in Step 2 (Business Owner Information)
-- ============================================
...
-- Stores identity verification documents
-- Created in Step 3 (Prove Your Identity)
-- ============================================
...
-- Created in Step 4 (Prove Your Business)
-- ============================================
...
-- Tracks final submission and approval status
-- Created in Step 6 (Submit for Verification)
-- ============================================
...
-- Tracks completion status of each setup step
-- Helps determine which step user should continue from
-- ============================================
...
-- NOTES:
-- ============================================
-- 1. All tables use 'tours_' prefix to avoid conflicts when merging
-- 2. Foreign keys reference tours_businesses table
-- 3. Status tracking:
--    - tours_businesses.status: Overall business status
--    - tours_setup_submissions.status: Submission/approval status
-- 4. File uploads are stored as URLs (paths to uploaded files)
-- 5. JSON fields (location_data) allow flexible location information
-- 6. Setup progress table helps resume flow if user leaves mid-process
-- 7. Email verification follows same pattern as stays system
-- 8. All timestamps use CURRENT_TIMESTAMP for created_at
-- 9. updated_at auto-updates on record modification
-- ============================================
